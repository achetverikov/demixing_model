...
in the root `TODO.md`. The concrete pre-refactor test cleanup and runtime-reduction work was carried out as part of this refactor; completed work is summarized in `HISTORY.md` and the root `TODO.md`.
...
- development-only transition and representation work is kept on the development branch and excluded from the production tree;
- historical findings live under `docs/history/wnm_transition/`;
- `continuous_density/` has been deleted;
- the production pytest configuration contains only maintained, integration, and legacy-surface categories;
