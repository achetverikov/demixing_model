...
adversarial but physically legal density-asymmetry curves (near-constant, high
means, tiny amplitudes), not just easy ones -- the batched form's arithmetic
differs, and that is exactly where an algebraically-equal rearrangement stops
...
def curve_families(rng):
    ...
    curves = {
        ...
        "ten_x_too_large": 10 * base,
        # Legal density-asymmetry range, but mean^2 >> var: cancellation risk.
        "high_mean": 0.95 + 0.01 * np.sin(np.pi * grid / 180.0),
    ...
    assert all(np.max(np.abs(curve)) <= 1.0 for curve in curves.values())
    return curves
...
@pytest.mark.parametrize("target_name", ["typical", "high_mean", "anticorrelated"])
...
def test_both_sides_must_be_centered_not_just_the_target():
    ...
    numerically wrong in float32 when the legal curve has a high mean and very
    little variation. The faulty form can return a negative loss for an identical
    curve, which 1 - CCC cannot legitimately do."""
    grid = np.linspace(2, 180, N_POINTS)
    high_mean_curve = 0.95 + 0.01 * np.sin(np.pi * grid / 180.0)
    predicted = high_mean_curve[None, :]

    centered_target, target_mean, target_var = do.target_statistics(high_mean_curve)
    ...
    assert float(batched(predicted, high_mean_curve)[0]) == pytest.approx(0.0, abs=1e-6)
