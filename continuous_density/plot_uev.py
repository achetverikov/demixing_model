#!/usr/bin/env python3
"""Plot raw 100k UEV mean-bias curves against the conditional density.
...
directly from the saved EM outcomes; no KDE or surface enters either curve.
"""
...
def build_summary(model_path: Path, reference_path: Path) -> pd.DataFrame:
    ...
    for component in (0, 1):
        ...
        for method, values in [('raw 100k', raw), ('conditional density', pred)]:
            frame = common.copy()
            frame['method'] = method
            frame['mean_bias'] = values
            frames.append(frame)
...
def plot_matrix(df: pd.DataFrame, dprime: float, out: Path):
    ...
    handles += [mlines.Line2D([], [], color='#555555', ls=ls, lw=2, label=method)
                for method, ls in METHOD_LS.items()]
    fig.legend(handles=handles, loc='lower center', ncol=4, fontsize=9)
    sd_ident = design_mod.UEV_SPAT_DIFF / dprime
    fig.suptitle(f'Raw 100k vs conditional density: spatial d′={dprime:g} '
...
def plot_averaged_uev(df: pd.DataFrame, dprime: float, out: Path):
    ...
    color_handles += [mlines.Line2D([], [], color=oranges[i], lw=2,
                      ...
                      for i, v in enumerate(sd1_vals)]
    method_handles = [mlines.Line2D([], [], color='#555555', ls=ls, lw=2, label=m)
...
def main():
    ...
    summary = build_summary(args.model, args.reference)
    summary.to_csv(args.out_dir / 'uev_raw100k_vs_density.csv', index=False)
